...
function Login() {
	...
	return (
		<>
			{!user&&!isConnected&&(<div className='w-screen h-screen flex flex-col gap-5 justify-center items-center py-20 mt-20 overflow-hidden select-none'>
					<Image
						src={!isConnected?'/metamask.svg':'/ceramic.svg'}
						width={300}
						height={300}
						alt={'Feather'}
					/>
					<p className='font-[400] relative z-10'>
						...
						Log in with your
						<span className="text-accent"> {!isConnected?'Metamask':'Ceramic Account'}</span>
						
					</p>
					<button 
						className="bg-accent p-2 rounded-xl w-[10%] relative z-10" 
						onClick={logInUser}>
							{!isConnected?'Connect':'Approve Ceramic'}
					</button>
